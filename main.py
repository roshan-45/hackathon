...
# output format
output = {}
output['process_id']="No Process ID with error"
            ...
            if l > 0:
                ...
                try:
                    ...
                    if isinstance(log_entries['log' + str(l - 1)]['information'], dict):
                        log_entries['log' + str(l - 1)]['information']['info'] = log_entries['log' + str(l - 1)]['information'][
                                                                                     'info'] + line
                    else:
...
def extract20entries(error_number, all_entries, parent_child_tree):
    prompt = ""
    if error_number < 20:
        print("Error occurred in the beginning of the file!")
    else:
...
def analyze_log(all_logs, error_logs, parent_child_tree):
    ...
    print("Log error was caused on : " + all_logs[full_log['log_no']]['timestamp'])
    output['timestamp'] = all_logs[full_log['log_no']]['timestamp']
    output["process_id"]=all_logs[full_log['log_no']]['project_id']
    print("Process-id"+all_logs[full_log['log_no']]['project_id'])
    ...
    if parent_child_tree == {}:  # no diag file
        ...
        prompt_template = PromptTemplate(
            ...
            template=(
                ...
                The log entry contains {type} in it. Use the log entry and previous log entries provided below and then analyze the error, give the root cause. 
        ...
        prompt_template = PromptTemplate(
            ...
            template=(
                '''Your task is to analyze the given log text, which usually contains a date, time, process_id, and details related to transportation management.
                The log entry contains {type} in it. Use the log entry, parent log entries, sibling log entries provided below and then analyze the error, give the root cause. 
                Also try to give the possible solution to solve the {type}.
                Log Entry: {log_entry}
    ...
    # Agent that runs the prompt
    agent = create_react_agent(llm=groq_llm, prompt=prompt_template, tools=tools)
...
def process_file():
    ...
    if error_logs:
        ...
        output['content'] = extractingSemiErrors(all_logs, parent_child_tree)
